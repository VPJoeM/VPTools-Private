...
echo "NCCL Test Helper Script - Version 0.20"
echo "--------------------------------------"
echo "Changes: Improved result parsing, maintained structure from v17, added support for all single node tests"
echo
...
# Ask if the user wants to see detailed test output
read -p "Do you want to see detailed test output? (y/N): " show_details
...
parse_results() {
    ...
    echo "$test_type Test: $test_name (Hosts: $hostnames)"
    
    # Extract average bus bandwidth
    local avg_bw=$(echo "$test_output" | grep "Avg bus bandwidth" | awk '{print $(NF-1)}')
    
    if [ -n "$avg_bw" ]; then
        echo "Avg bus bandwidth: $avg_bw GB/s"
    else
        echo "Avg bus bandwidth: Not available"
    fi
    echo "Description: ${TEST_DESCRIPTIONS[$test_name]}"
    
    if [[ "$show_details" == "y" ]]; then
        echo "Devices Used:"
        echo "$test_output" | grep "Rank" | sed 's/^#  //'
        echo
        echo "Test Results:"
        echo "$test_output" | awk '
        /^[^#]/ && NF == 11 {
            if (NR == 1) {
                print "Size (B)\tOut-of-place (GB/s)\tIn-place (GB/s)"
                print "--------\t-------------------\t----------------"
            }
            printf "%s\t%.2f\t\t\t%.2f\n", $1, $7, $10
        }'
...
TEST_DESCRIPTIONS["all_gather_perf"]="Gathers data from all GPUs to all GPUs"
TEST_DESCRIPTIONS["all_reduce_perf"]="Reduces data on all GPUs"
TEST_DESCRIPTIONS["broadcast_perf"]="Broadcasts data from one GPU to all others"
TEST_DESCRIPTIONS["reduce_perf"]="Reduces data to a single GPU"
TEST_DESCRIPTIONS["reduce_scatter_perf"]="Reduces data and scatters the result"
TEST_DESCRIPTIONS["scatter_perf"]="Scatters data from one GPU to all others"
TEST_DESCRIPTIONS["sendrecv_perf"]="Sends and receives data between pairs of GPUs"
TEST_DESCRIPTIONS["alltoall_perf"]="Exchanges data between all GPUs"
TEST_DESCRIPTIONS["gather_perf"]="Gathers data from all GPUs to one GPU"
TEST_DESCRIPTIONS["hypercube_perf"]="Performs hypercube-based collective operations"

echo "Single Node NCCL Test Results"
echo "-----------------------------"
SINGLE_NODE_TESTS=("alltoall_perf" "broadcast_perf" "gather_perf" "hypercube_perf" "reduce_perf" "reduce_scatter_perf" "scatter_perf" "sendrecv_perf")

for hostname in "${hostnames[@]}"; do
    echo "Results for host: $hostname"
    for test in "${SINGLE_NODE_TESTS[@]}"; do
        test_output=$(echo "$NCCL_TEST_RESULTS" | sed -n "/Results for.*$test.*$hostname/,/Average bus bandwidth/p")
        if [ -n "$test_output" ]; then
            parse_results "$test_output" "$test" "Single Node" "$hostname"
        else
            echo "No results found for single node test ($test) on $hostname"
        fi
    done
    echo
done
...
echo
echo "Cluster-wide NCCL Test Results"
echo "------------------------------"
CLUSTER_TESTS=("all_gather_perf" "all_reduce_perf" "broadcast_perf" "reduce_perf" "reduce_scatter_perf" "scatter_perf" "sendrecv_perf")

for test in "${CLUSTER_TESTS[@]}"; do
    test_output=$(echo "$NCCL_TEST_RESULTS" | sed -n "/Results for $test:/,/Average bus bandwidth/p")
    if [ -n "$test_output" ]; then
        parse_results "$test_output" "$test" "Cluster-wide" "${hostnames[*]}"
    else
        echo "No results found for cluster-wide test ($test)"
    fi
done
