...
echo "NCCL Test Helper Script - Version 0.17"
echo "--------------------------------------"
echo "Changes: Removed duplicate descriptions, removed local mpirun check, added hostnames to test titles"
echo
...
echo "-----------------------------"
alltoall_output=$(echo "$NCCL_TEST_RESULTS" | sed -n '/Results for.*alltoall_perf/,/Average bus bandwidth/p')
if [ -n "$alltoall_output" ]; then
    parse_results "$alltoall_output" "alltoall_perf" "Single Node" "${hostnames[0]}"
else
    echo "No results found for single node test (alltoall_perf)"
fi

echo
echo "Cluster-wide NCCL Test Results"
echo "------------------------------"
TESTS=("all_gather_perf" "all_reduce_perf" "broadcast_perf" "reduce_perf" "reduce_scatter_perf" "scatter_perf" "sendrecv_perf")
